...
class Evaluator:
        ...
        if isinstance(self.labels, dict):
            ...
            for label, name in self.labels.items():
                ...
                if not hasattr(label, "__iter__"):
                    ...
                    self.confusion_matrix.set_reference(
    ...
    def to_array(self):
        ...
        result_metrics = sorted(
            self.result[list(self.result.keys())[0]].keys())
...
def aggregate_scores(test_ref_pairs,
    ...
    p = Pool(num_threads)
    all_res = p.map(run_evaluation, zip(
        test, ref, [evaluator]*len(ref), [metric_kwargs]*len(ref)))
    p.close()
    p.join()
    ...
    if json_output_file is not None:
        ...
        json_dict["id"] = hashlib.md5(json.dumps(
            json_dict).encode("utf-8")).hexdigest()[:12]
...
def evaluate_folder(folder_with_gts: str, folder_with_predictions: str, labels: tuple, **metric_kwargs):
    ...
    files_pred = subfiles(folder_with_predictions,
                          suffix=".nii.gz", join=False)
    assert all([i in files_pred for i in files_gt]
               ), "files missing in folder_with_predictions"
    assert all([i in files_gt for i in files_pred]
               ), "files missing in folder_with_gts"
    test_ref_pairs = [(join(folder_with_predictions, i),
                       join(folder_with_gts, i)) for i in files_pred]
