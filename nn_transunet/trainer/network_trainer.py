...
def warmup_poly_lr(epoch, max_epochs, warmup_epochs, initial_lr, exponent=0.9):
    if epoch < warmup_epochs:
        return initial_lr * (float(epoch) / float(max(1.0, warmup_epochs)))
    epoch_rel = epoch - warmup_epochs
    max_epochs_rel = max_epochs - warmup_epochs
    return initial_lr * (1 - epoch_rel / max_epochs_rel) ** exponent
...
class NetworkTrainer(object):
    ...
    def do_split(self):
        ...
        if isinstance(self.fold, str) and self.fold.startswith("all"):
            ...
            if  self.fold == "all":
                ...
                cur_num = int(len(all_keys_sorted) * proportion)
                tr_keys = val_keys = all_keys_sorted[:cur_num]
                assert 1==2, (tr_keys, '--------', all_keys_sorted)
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        save_this = {
            'epoch': self.epoch + 1, # actually saved epoch + 1...
    ...
    #             zoomed_output = F.interpolate(output, (256, 256),  mode='bilinear', align_corners=True)
    #             zoomed_data = F.interpolate(data, (256, 256), mode='bilinear', align_corners=True)
    #             del data

    #             gatedcrf = self.gatecrfloss(
    #                 torch.softmax(zoomed_output, dim=1),
    ...
    #                 256,
    #             )["loss"]
    #             pce = self.loss(output, target)
    #             l = pce + 0.1 * (self.epoch / 3000)* gatedcrf
    #             del zoomed_output
    #             del zoomed_data
    ...
    #             output = self.network(data)
    #             pce = self.loss(output, target)
    #             mean_std = torch.mean(torch.std(data * torch.softmax(output, dim=1), dim=[2, 3]))
    #             l = pce + 0.1 * mean_std
    #             del data
    #         if do_backprop:
    #             self.amp_grad_scaler.scale(l).backward()
